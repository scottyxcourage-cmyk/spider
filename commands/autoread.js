...
module.exports = async (sock, chatId, message, args) => {
    ...
    if (!sub) return reply(sock, chatId, `👁️ *Auto-Read*\nStatus: ${d.enabled?'✅ ON':'❌ OFF'}\n\n.autoread on\n.autoread off`, message);
    if (sub==='on')  { save({enabled:true});  return reply(sock, chatId, '✅ Auto-read enabled!', message); }
    if (sub==='off') { save({enabled:false}); return reply(sock, chatId, '❌ Auto-read disabled.', message); }
};
